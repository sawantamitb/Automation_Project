...
public class pdfListener extends TestBase implements ITestListener 
{
	...
	public pdfListener() 
	{
		...
		PropertyReading objConfigPropertyReading = new PropertyReading();
	...
	public void onTestFailure(ITestResult result) {
		 ...
		 * System.currentTimeMillis() + ".png"; try { TestBase.getScreenshot(); }
		...
		if (throwable != null) {
			...
			Chunk imdb = new Chunk("[SCREEN SHOT]",new Font(Font.TIMES_ROMAN,Font.DEFAULTSIZE,Font.UNDEFINED));
	...
	public void onStart(ITestContext context) {
		log("onStart("+context+")");
		try {
			PdfWriter.getInstance(this.document, new FileOutputStream(System.getProperty("user.dir") + File.separator+extentreport +context.getName()+".pdf"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		this.document.open();
